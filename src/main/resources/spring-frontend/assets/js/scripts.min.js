...
(function ($) {
    ...
    $(function () {
            ...
            return {
                init: function () {
                    $('[data-refreshshow]').on('click change', function (e) {
                        ...
                        function getDataMain(address, data, type) {
                            var scroll = $(window).scrollTop();
                            $.ajax({
                                url: address,
                                type: 'GET',
                                ...
                                complete: function (result) {
                                    if (result.status === 200) {
                                        ...
                                        var $Cards = $('.Cards_refresh');
                                        if (type === 'changedate') {
                                            $Cards.find('.card-recent').remove();
                                        }
                                        if ($Cards.find('.card-recent').length) {
                                            $Cards.find('.card-recent').last().after(data);
                                        } else {
                                            $Cards.prepend(data);
                                        }
                                        $(window).scrollTop(scroll);
                        ...
                        switch (data) {
                            ...
                            case'genre':
                                getData('/books/genre', {
                                    genreName: $this.data('refreshtagname'),
                                    offset: function () {
                                        let oldOffset = $this.data('refreshoffset');
                                        if (oldOffset < 1) {
                                            $this.data('refreshoffset', ++oldOffset);
                                ...
                                break;
                            case'genreFolder':
                                getData('/books/folder/genre', {
                                    folderId: $this.data('refreshid'),
                                    offset: function () {
                                        let oldOffset = $this.data('refreshoffset');
                                        if (oldOffset < 1) {
                                            $this.data('refreshoffset', ++oldOffset);
                            ...
                            case'author':
                                getData('/books/author', {
                                    authorName: $this.data('refreshauthorname'),
                                    offset: function () {
                                        let oldOffset = $this.data('refreshoffset');
                                        if (oldOffset < 1) {
                                            $this.data('refreshoffset', ++oldOffset);
                            ...
                            case'tag':
                                getData('/books/tags', {
                                    tagName: $this.data('refreshtagname'),
                                    offset: function () {
                                        let oldOffset = $this.data('refreshoffset');
                                        if (oldOffset < 1) {
                                            $this.data('refreshoffset', ++oldOffset);
            ...
            return {
                init: function () {
                    ...
                    $('.Comments-addComment form').on('submit', function (e) {
                        ...
                        if (!error) {
                            Login().postData('/bookReview/' + $this.data('bookid') + '/' + $this.find('.Comments-review').val(), {
                                ...
                                if (result) {
                                    ...
                                    $(".Product-comments").get(0).scrollIntoView();
                                }
                            })
                        }
                    });
                    $('[data-likeid]').on('click', function (e) {
                        ...
                        ;
                        data.slug = $this.data('likeid').split('/')[0];
                        data.reviewid = $this.data('likeid').split('/')[1];
            ...
            return {
                init: function () {
                    ...
                    $ProductAssesment.find('.Rating input').on('change', function (e) {
                        ...
                        Login().postData('/rateBook/' + $this.data('bookid'), {
        ...
        $( document ).ready(function() {
            function getDataCount(address, data) {
                $.ajax({
                    url: address,
                    type: 'GET',
                    dataType: 'json',
                    data: data,
                    complete: function (result) {
                        if (document.cookie.split(';').filter((item) => item.includes('cartContents')).length) {
                            console.log('The cookie - cartContents')
                        }
                    }
                });
            };
            getDataCount('/books/postponed/count');
        });
    });
})(jQuery);
